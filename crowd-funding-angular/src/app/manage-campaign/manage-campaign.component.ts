...
export class ManageCampaignComponent implements OnInit {
  ...
  constructor(private fundsService:FundsService, private authService:AuthService, private router:Router) { }
    ...
    if(localStorage.getItem('token')){
      this.authService.getUserId().subscribe((data:any)=>{
        if(data.status=="1"){
          ...
          this.fundsService.manageCampaigns(data.user_id).subscribe((data:any)=>{
            this.funds=data;
            console.log(data);
          });
